...
session_start();
	...
	if ( $createEntry == 1 ) {
		...
		$name = replaceBlank( sanitizeText( $_POST[ "name" ] ) );
		$lastname = replaceBlank( sanitizeText( $_POST[ "lastname" ] ) );
		$birthday = replaceBlank( sanitizeText( $_POST[ "birthday" ] ) );
		$deathday = replaceBlank( sanitizeText( $_POST[ "deathday" ] ) );
		$occupation = replaceBlank( sanitizeText( $_POST[ "occupation" ] ) );
		$graveyard = replaceBlank( sanitizeText( $_POST[ "graveyard" ] ) );
		$description = replaceBlank( sanitizeText( $_POST[ "description" ] ) );
		...
		insertIntoActivity($activities, 'book');
		
		//database creating record in funeral & activity
		if ( $_FILES[ "image" ]["name"] != "" && isset($_FILES[ "image" ]))
			$avatarUrl = uploadAvatar();
		else
			$avatarUrl = "/images/defaultDead.png";
		
		$qry = $db->prepare( "INSERT INTO book (ID, entry_key, name, lastname, birthday, deathday, occupation, graveyard, description, avatar_url) VALUES (NULL, :entry_key, :name, :lastname, :birthday, :deathday, :occupation, :graveyard, :description, :avatarUrl)" );
		$qry->bindParam( ":entry_key", $activities );
		$qry->bindParam( ":name", $name );
		$qry->bindParam( ":lastname", $lastname );
		$qry->bindParam( ":birthday", $birthday );
		$qry->bindParam( ":deathday", $deathday );
		$qry->bindParam( ":occupation", $occupation );
		$qry->bindParam( ":graveyard", $graveyard );
    ...
    <div class="row justify-content-center">
      <div class="col-11 col-sm-9 col-md-7 col-lg-6 col-xl-5 text-center p-0 mt-3 mb-2 bg-dark">
        <div class="card px-3 pt-4 pb-0 mt-3 mb-3 bg-dark">
          <h2 id="heading" class="text-warning">Dodaj osobę zmarłą</h2>
          <p>Wypełnij wszystkie dane oznaczone *gwiazdką</p>
          <form id="msform" method="post" enctype="multipart/form-data">
            <input type="hidden" name="createEntry" value="1" />
            <!-- progressbar -->
            <ul id="progressbar">
            ...
            <fieldset class="bg-dark" id="jeden">
              <div class="form-card bg-dark" id="dwa">
                ...
                <input id="log" type="text" name="name" placeholder="Imię..." class="compInput" />
                <label class="fieldlabels">Nazwisko zmarłego *</label>
                <input id="naz" type="text" name="lastname" placeholder="Nazwisko..."class="compInput" />
                <label class="fieldlabels">Data urodzenia</label>
                <input type="date" name="birthday" placeholder="Data urodzenia..." />
                <label class="fieldlabels">Data śmierci </label>
                <input type="date" name="deathday" placeholder="Data śmierci..." />
              </div>
            ...
            <fieldset class="bg-dark  ">
              <div class="form-card bg-dark">
                ...
                <label class="fieldlabels">Zawód </label>
                <input type="text" name="occupation" placeholder="Zawód..." />
				 <div class="row no-gutters">
               <input id="graveyardCheckID" style="width: 10%" type="checkbox" name="graveyard" value="1" checked="checked"> <p style="width: 90%">Spoczywa na Starym Cmentarzu w Jarosławiu </p>
				</div>
				  <div class="row d-none px-3" id="graveyardID">
				  <label class="fieldlabels">Cmentarz </label><br>
                <input  type="text" name="graveyard" placeholder="Cmentarz..." /> 
					  </div>
            ...
            <fieldset class="bg-dark  ">
              <div class="form-card bg-dark">
                ...
                <div class="row no-gutter my-5 py-5 mx-auto">
                  <div class="file-drop-area text-center mx-auto border border-secondary rounded" id="plik-menu" > 
					<span class="choose-file-button" style="z-index: 0">Wybierz plik</span> 
					<span class="file-message" style="z-index: 0" id="avbat">lub przeciągnij go tutaj</span>
                    <input type="file" id="avatar" name="image" class="file-input" style="z-index: 999" accept=".jfif,.jpg,.jpeg,.png,.gif">
                  </div>
                  <div id="divImageMediaPreview"> </div>
                </div>
              </div>
              <input type="submit" name="entry_Button" class="next action-button" value="Zakończ" />
              <input type="button" name="previous" class="previous action-button-previous" value="Poprzedni" />
            </fieldset>
            <fieldset class="bg-dark  ">
              <div class="form-card bg-dark">
...
$(document).ready(function() {
  $('#graveyardCheckID').change(function() {
    if ($('#graveyardCheckID').attr('value') == 0) {
      $('#graveyardID').addClass('d-none');
      $('#graveyardID').removeClass('d-block');
      $('#graveyardCheckID').attr('value', 1);
    } else {
      $('#graveyardID').addClass('d-block');
      $('#graveyardID').removeClass('d-none');
      $('#graveyardCheckID').attr('value', 0);
    }
  });
});
</script>